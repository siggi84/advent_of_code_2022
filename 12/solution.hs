...
parseInput s = (mapGrid, start, end)
  where
    charGrid = words s
    n = length charGrid
    m = length (head charGrid)
    charFinder c i (g:gs) =
      case elemIndex c g of
        Just n -> (i, n)
    ...
    flattened = map converter $ concat charGrid
...
partn (g, start, stop) isPart1 = res
  where
    bfs :: [Coord] -> [Coord] -> Int -> Int
    bfs current visited l = res
      where
        neighbors :: Coord -> [Coord]
        neighbors (i, j) =
          filter
            (\c ->
               case (Map.lookup (i, j) g, Map.lookup c g) of
                 (Just sv, Just dv) -> sv - dv < 2
                 _ -> False)
            [(i - 1, j), (i + 1, j), (i, j - 1), (i, j + 1)]
        enqued :: [Coord]
        enqued = filter (`notElem` visited) $ nub $ concatMap neighbors current
        condition1 = start `elem` current
        condition2 =
          any
            (\c ->
        ...
        res =
          if condition
            then l
            else bfs enqued (visited ++ current) (l + 1)
    res = bfs [stop] [stop] 0
